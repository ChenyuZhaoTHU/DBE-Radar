...
def aoa_capon(x, steering_vector, magnitude=False, qr=False):
    ...
    # Add regularization to Rxx
    Rxx = Rxx + 0.001 * np.trace(Rxx) * np.eye(Rxx.shape[0]) / Rxx.shape[0]
    ...
    if qr:
        Q, R = np.linalg.qr(Rxx)
        Rxx_inv = np.linalg.inv(R)
    else:
        # Rxx = forward_backward_avg(Rxx)
        Rxx_inv = np.linalg.inv(Rxx)
...
def gen_steering_vec(ang_est_range, ang_est_resolution, num_ant, distance: float = 1.):
    ...
    Args:
        ...
        distance (float): The distance between the antennas in the array. Default is 1(=2pi/lamda * [lamda/2]).
    ...
    for kk in range(num_vec):
        for jj in range(num_ant):
            mag = -1 * np.pi * jj * distance * np.sin((-ang_est_range + kk * ang_est_resolution) * np.pi / 180)

            # steering_vectors[kk, jj] = np.complex64(real + 1j*imag)
            steering_vectors[kk, jj] = np.exp(1j * mag)
